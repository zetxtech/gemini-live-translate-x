    ...
    unsafe extern "system" fn QueryInterface(
        ...
        unsafe {
            ...
            *interface = if *iid == <AsyncActionCompletedHandler as windows_core::Interface>::IID
            ...
            } else if is_imarshal_iid(iid) {
                (*this).count.add_ref();
                return free_threaded_marshaler(
...
// The free-threaded marshaler bindings only exist on Windows targets,
// so keep those references behind cfg to allow cross-platform builds.
#[cfg(windows)]
unsafe fn is_imarshal_iid(iid: *const windows_core::GUID) -> bool {
    *iid == <windows_core::imp::IMarshal as windows_core::Interface>::IID
}
#[cfg(not(windows))]
unsafe fn is_imarshal_iid(_iid: *const windows_core::GUID) -> bool {
    false
}

#[cfg(windows)]
unsafe fn free_threaded_marshaler(
    outer: windows_core::IUnknown,
    interface: *mut *mut core::ffi::c_void,
) -> windows_core::HRESULT {
    windows_core::imp::marshaler(outer, interface)
}
#[cfg(not(windows))]
unsafe fn free_threaded_marshaler(
    _outer: windows_core::IUnknown,
    _interface: *mut *mut core::ffi::c_void,
) -> windows_core::HRESULT {
    windows_core::HRESULT(-2147467261)
}